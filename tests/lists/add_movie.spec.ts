...
test.afterEach(async ({ request }) => {
  await request.delete(`/3/list/${listId}/remove_item`, {
    data: { media_id: 26 },
  });
});
...
test("Validar adição de filme em uma lista com sucesso", async ({
  request,
}) => {
  const response = await request.post(`/3/list/${listId}/add_item`, {
    data: { media_id: 26 },
  });
  const responseBody = await response.json();
  ...
  expect(responseBody.status_message).toBe(
    "The item/record was updated successfully.",
  );
  expect(responseBody.success).toBeTruthy();
});

test("Validar erro ao tentar adicionar um filme mais de uma vez na lista", async ({
  request,
}) => {
...
test("Validar erro ao tentar adicionar filme inválido na lista", async ({
  request,
}) => {
  const response = await request.post(`/3/list/${listId}/add_item`, {
    data: { media_id: "inv" },
